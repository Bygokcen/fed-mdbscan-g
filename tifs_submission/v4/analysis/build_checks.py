...
def build(root, tex_table, derived):
    ...
    (gen/'flame_sizes.tex').write_text(tex_table(
        caption='FLAME retained sizes in adversary-free main cells, counted from saved accepted identities. Each row contains three seeds and 90 rounds.',
        label='tab:flame_sizes', header=['Dataset',r'$\alpha$','Mean size','Range','Size 46'],
        rows=sizes,colspec='llrrr',
        notes=r'The frozen normal path retains one non-noise cluster; historical cluster labels were not saved. The minimum is attained in 719/990 rounds. Counts are not reconstructed from BER.'))
    ...
    for label,suffix in [('Min-Max','scenario_cut_minmax'),('Patch','scenario_cut_backdoor'),('All attacked',None)]:
        ...
        grows.append([label,str(len(rs)),str(sum(x<=2 for x in values)),f'{st.median(values):.6f}',f'{max(values):.6f}'])
    assert grows[-1][2]=='35'
    (gen/'gamma.tex').write_text(tex_table(
        caption=r'Radial ratio of the initial acceptance pool on distinct attacked checkpoint matrices. The threshold is $\tau=2$.',
        label='tab:gamma',header=['Attack','Matrices',r'$\Gamma\leq2$','Median','Maximum'],rows=grows,colspec='lrrrr',
        notes=r'Both gate evaluations reuse each matrix and are counted once. In all 36 matrices, $B_0$ contains all 90 participants. The exception is Fashion, seed 137, patch, round 29.'))
    ...
    methods={'norm_clip':'Norm clipping','coord_median':'Coordinate median','krum_bound30':'Multi-Krum','fltrust_normalized':'FLTrust','flame_hdbscan':'FLAME','fed_mdbscan_g':'Fed-MDBSCAN-G'}
    paired=[]
    ...
    (gen/'benign_seed_summary.tex').write_text(tex_table(
        caption=r'Benign utility cost over paired dataset--seed units, in percentage points. Minima and maxima span datasets and seeds, not confidence intervals.',
        label='tab:seed_cost',header=[r'$\alpha$','Rule','Pairs','Mean','Min.','Max.'],rows=srows,colspec='llrrrr',
        notes='The artifact includes each per-seed difference. Pairing checks dataset, partition, initial model and participation schedule hashes. Coverage varies by rule and concentration.'))
