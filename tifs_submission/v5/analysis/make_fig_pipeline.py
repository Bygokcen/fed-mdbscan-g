...
def draw(out_stem, apply_style):
    ...
    rows = [  # (y, h, title, body, color)
        ...
        (0.085, 0.105, r"Combine: $B=B_0\setminus$(rejected groups)",
    ...
    axA.text(xc + 0.012, (rows[2][0] + rows[3][0] + rows[3][1]) / 2, "gate open", fontsize=6, color=C_S3, va="center")
    # aggregate + bypass
    ...
    axB.set_title("Illustrative 2-D example (synthetic points)", fontsize=7, loc="left", pad=3)
    from matplotlib.lines import Line2D
    h = [Line2D([], [], marker="o", ls="", ms=3.5, mfc=C_ACC, mec=C_ACC, label="honest, accepted"),
         Line2D([], [], marker="o", ls="", ms=3.5, mfc="white", mec=C_S1, label="honest, excluded"),
         Line2D([], [], marker="^", ls="", ms=3.8, mfc="white", mec=C_S1, label="attacker, excluded"),
