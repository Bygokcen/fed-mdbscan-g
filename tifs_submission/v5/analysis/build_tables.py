...
def main():
    ...
    t1 = tex_table(
        ...
        notes=(r"Honest client--round submissions rejected with no adversary present; three "
               ...
               r"per cell). Fashion denotes Fashion-MNIST; dashes mark conditions outside the "
               r"planned coverage. FLTrust counts zero-weight submissions. Norm clipping and the "
               r"coordinate-wise median reject no client, so their BER is zero by construction; "
               r"their benign cost appears in Table~\ref{tab:buc}."),
        star=True)
    ...
    t2 = tex_table(
        ...
        notes=(r"Final-round accuracy of the undefended uniform mean minus that of the defense, "
               r"in the runs of Table~\ref{tab:ber}; positive values denote lower accuracy than "
               r"the uniform mean. Means over three seeds; per-seed ranges are given in the "
               r"supplement."),
        star=True)
    ...
    t3 = tex_table(
        ...
        notes=(r"$J=\mathrm{TPR}-\mathrm{FPR}$ (Youden's index); $J<0$ means honest clients are "
               r"rejected in preference to attackers. Benefit is defense accuracy minus undefended "
               r"accuracy in the same condition. Each row pools rule~$\times$~dataset~$\times$~"
               r"condition cells of the four rejecting rules; family coverage differs, so the rows "
               r"are descriptive averages."))
    ...
    t5 = tex_table(
        ...
        notes=(r"The denominator is adversary-free rounds, so every alarm is false. "
               r"Fed-MDBSCAN-G is the only evaluated rule with a round-level alarm; the other "
               r"rules have no alarm channel, and no rate is reported for them."))
    ...
    t6 = tex_table(
        ...
        notes=(r"Cluster structure, geometry and activation memory are held fixed. Entries are "
               r"honest/attacker rejections as client--checkpoint counts after the safety valve. "
               r"P0 is the deployed policy (clusters only); P1 also submits unclustered "
               r"low-density updates to the same test as singletons; P2 excludes them outright. "
               r"Each attacked block contains 1{,}296 honest and 324 attacker observations; each "
               r"attack-disabled block contains 1{,}620 honest observations."))
