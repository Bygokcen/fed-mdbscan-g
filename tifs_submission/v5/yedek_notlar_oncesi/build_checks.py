...
def build(root, tex_table, derived):
    ...
    (gen/'flame_sizes.tex').write_text(tex_table(
        ...
        notes=r'Counted from saved accepted identities in the adversary-free main cells; each row contains three seeds and 90 rounds. Size 46 counts rounds at the minimum admissible size. The frozen normal path retains one non-noise cluster; historical cluster labels were not saved. The minimum is attained in 719/990 rounds. Counts are not reconstructed from BER.'))
